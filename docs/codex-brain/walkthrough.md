...
- 사용자는 현재 제휴 섹션이 카드 수에 비해 화면이 허전하고, 상품보다 설명 패널이 더 크게 느껴져 전체적으로 아쉽다고 피드백했다.
- 직전 구조는 왼쪽 설명 카드와 오른쪽 상품 보드가 분리돼 보여, 실제 상품 비교 영역보다 안내 영역이 더 먼저 눈에 들어오는 문제가 있었다.

### 반영 내용

- [App.tsx](C:/projects/imomguide_remote_20260319/src/App.tsx)
  - 제휴 섹션 상단을 `타이틀 + 카테고리 배지` 헤더로 다시 구성해 첫 화면부터 섹션 목적이 바로 보이도록 조정했다.
  - 기존 `왼쪽 안내 / 오른쪽 상품` 비중을 뒤집어, 왼쪽에는 넓은 상품 보드를 두고 오른쪽에는 좁은 사이드 레일만 남겼다.
  - 상품 카드 4개는 모두 `설명 + iframe 위젯`이 같은 카드 안에서 균형 있게 보이도록 재정렬했고, 내부에 `빠른 비교`, `외부 링크` 태그를 추가해 카드 하단 공백을 줄였다.
  - 사이드 레일은 `바로 보는 안내`, 카테고리 타일, CTA 2개, 광고·제휴 안내 순으로 정리해 보조 정보만 압축적으로 모이게 했다.
...
- `npm run build` 통과
  - `dist/index.html`
  - `dist/assets/index-BoModLpz.css`
  - `dist/assets/index-CB5G_Dcg.js`

### 결과 요약

- 제휴 섹션의 주인공이 설명 박스에서 상품 비교 보드로 이동하면서, 전체 화면이 더 빽빽하고 균형감 있게 보이도록 정리됐다.
- 빈 공간이 크게 느껴지던 왼쪽 면적은 줄이고, 실제 클릭 가능한 상품 카드가 먼저 들어오도록 구조를 재배치했다.
...
- 사용자는 직전 제휴 섹션에서도 오른쪽 하단 쪽이 비어 보인다고 피드백했다.
- 기존 구조는 `소개 패널 + 상품 보드`가 서로 다른 높이로 분리돼 있어, 카드 수와 무관하게 섹션 하단 일부가 덜 채워진 느낌을 남겼다.

### 반영 내용

- [App.tsx](C:/projects/imomguide_remote_20260319/src/App.tsx)
  - 메인 제휴 영역을 `왼쪽 소개 타일 1개 + 오른쪽 보드 헤더 1개 + 상품 카드 4개`로 채워지는 벤토 구조로 다시 설계했다.
  - 왼쪽 타일은 `xl:row-span-3`으로 전체 높이를 함께 먹도록 바꿔, 섹션 하단에 어색한 잔여 공간이 생기지 않게 했다.
  - 오른쪽은 `사이드 추천 보드` 헤더 아래에 2x2 카드 그리드를 고정해, 우측 하단도 카드로 자연스럽게 채워지게 만들었다.
  - 왼쪽 타일의 중복 제목은 `바로 보는 추천`으로 정리해 상단 섹션 타이틀과 충돌하지 않게 했다.
...
- 사용자는 `100%` 화면에서 여전히 좌우 사이드 광고가 첫 화면에 보이지 않는다고 피드백했다.
- 직전 수정은 `1440px` 이상에서만 노출되도록 유지하고 있었고, `top-1/2` 중심 정렬이라 화면 상단 기준으로는 존재감이 약했다.

### 반영 내용

- [coupang-side-banner.tsx](C:/projects/imomguide_remote_20260319/src/components/coupang-side-banner.tsx)
  - 노출 시작 기준을 `min-[1440px]`에서 `lg`로 더 낮췄다.
  - 배너 래퍼 크기와 scale을 `lg / xl / 2xl / 1800px / 2100px` 구간으로 더 잘게 나눠, 작은 화면에서도 축소판이 먼저 보이게 조정했다.
  - 세로 기준점도 `top-1/2`에서 `top-28`로 바꿔 첫 화면 상단 구간에서 배너가 바로 눈에 들어오게 했다.
- [App.tsx](C:/projects/imomguide_remote_20260319/src/App.tsx)
  - 좌우 오프셋을 축소폭 기준으로 다시 계산해, 작은 레일 배너가 콘텐츠 여백 안쪽에 더 자연스럽게 붙도록 조정했다.
...
- `npm run build` 통과
  - `dist/index.html`
  - `dist/assets/index-CTdqreV9.css`
  - `dist/assets/index-Cphdh4-P.js`
