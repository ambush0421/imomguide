...
- [x] 현재 입주업종/지식산업센터 규칙셋과 최신 고시 비교
- [x] `docs/codex-brain/implementation_plan.md` 승인 대기용 계획 작성
- [x] 사용자 승인 대기
- [x] 최신 고시 기준으로 법적 근거 메타데이터(`문서명`, `문서번호`, `날짜`, `링크`) 갱신
- [x] 결과/가이드/법령 라이브러리의 출처 문구를 최신 고시 기준으로 정렬
- [x] 최신 고시에서 다시 확인된 운영 조건(심의, 제조시설, 입주계약/임대차 제한) 노출 범위 반영
- [x] 관련 테스트 및 정적 산출물 점검
- [x] `npm run lint`
- [x] `npm run test`
- [x] `npm run build`
- [x] `docs/codex-brain/walkthrough.md` 기록
